...
class RecipeView extends View {
  ...
  _data;
  _errorMessage = `Sorry, we could not find the recipe you were looking for`;
  _successMessage = '';
  ...
  addHandlerUpdateServingHandler(handler) {
    this._parentEl.addEventListener('click', function (e) {
      const btn = e.target.closest('.btn--update-servings');
      if (!btn) return;

      const updateTo = btn.dataset.update;
      if (+updateTo > 0 && +updateTo < 1000) handler(+updateTo);
    });
  }
